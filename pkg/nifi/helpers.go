...
func WithBearerToken(token string) ClientOption {
	return func(client *Client) error {
		if token == "" || token != strings.TrimSpace(token) || strings.ContainsAny(token, "\r\n") {
			return errors.New("nifi: bearer token must be non-empty and contain no surrounding whitespace or newlines")
		}
...
// response. NiFi releases differ in whether successful payloads are declared
// as an explicit 2xx response or as the OpenAPI default response.
