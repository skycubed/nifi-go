// Code generated by go-swagger; DO NOT EDIT.

package snippets

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"
	"net/http"
	"time"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/runtime"
	cr "github.com/go-openapi/runtime/client"
	"github.com/go-openapi/strfmt"

	"github.com/skycubed/nifi-go/pkg/nifi/models"
)

// NewCreateSnippetParams creates a new CreateSnippetParams object,
// with the default timeout for this client.
//
// Default values are not hydrated, since defaults are normally applied by the API server side.
//
// To enforce default values in parameter, use SetDefaults or WithDefaults.
func NewCreateSnippetParams() *CreateSnippetParams {
	return &CreateSnippetParams{
		timeout: cr.DefaultTimeout,
	}
}

// NewCreateSnippetParamsWithTimeout creates a new CreateSnippetParams object
// with the ability to set a timeout on a request.
func NewCreateSnippetParamsWithTimeout(timeout time.Duration) *CreateSnippetParams {
	return &CreateSnippetParams{
		timeout: timeout,
	}
}

// NewCreateSnippetParamsWithContext creates a new CreateSnippetParams object
// with the ability to set a context for a request.
func NewCreateSnippetParamsWithContext(ctx context.Context) *CreateSnippetParams {
	return &CreateSnippetParams{
		Context: ctx,
	}
}

// NewCreateSnippetParamsWithHTTPClient creates a new CreateSnippetParams object
// with the ability to set a custom HTTPClient for a request.
func NewCreateSnippetParamsWithHTTPClient(client *http.Client) *CreateSnippetParams {
	return &CreateSnippetParams{
		HTTPClient: client,
	}
}

/* CreateSnippetParams contains all the parameters to send to the API endpoint
   for the create snippet operation.

   Typically these are written to a http.Request.
*/
type CreateSnippetParams struct {

	/* Body.

	   The snippet configuration details.
	*/
	Body *models.SnippetEntity

	timeout    time.Duration
	Context    context.Context
	HTTPClient *http.Client
}

// WithDefaults hydrates default values in the create snippet params (not the query body).
//
// All values with no default are reset to their zero value.
func (o *CreateSnippetParams) WithDefaults() *CreateSnippetParams {
	o.SetDefaults()
	return o
}

// SetDefaults hydrates default values in the create snippet params (not the query body).
//
// All values with no default are reset to their zero value.
func (o *CreateSnippetParams) SetDefaults() {
	// no default values defined for this parameter
}

// WithTimeout adds the timeout to the create snippet params
func (o *CreateSnippetParams) WithTimeout(timeout time.Duration) *CreateSnippetParams {
	o.SetTimeout(timeout)
	return o
}

// SetTimeout adds the timeout to the create snippet params
func (o *CreateSnippetParams) SetTimeout(timeout time.Duration) {
	o.timeout = timeout
}

// WithContext adds the context to the create snippet params
func (o *CreateSnippetParams) WithContext(ctx context.Context) *CreateSnippetParams {
	o.SetContext(ctx)
	return o
}

// SetContext adds the context to the create snippet params
func (o *CreateSnippetParams) SetContext(ctx context.Context) {
	o.Context = ctx
}

// WithHTTPClient adds the HTTPClient to the create snippet params
func (o *CreateSnippetParams) WithHTTPClient(client *http.Client) *CreateSnippetParams {
	o.SetHTTPClient(client)
	return o
}

// SetHTTPClient adds the HTTPClient to the create snippet params
func (o *CreateSnippetParams) SetHTTPClient(client *http.Client) {
	o.HTTPClient = client
}

// WithBody adds the body to the create snippet params
func (o *CreateSnippetParams) WithBody(body *models.SnippetEntity) *CreateSnippetParams {
	o.SetBody(body)
	return o
}

// SetBody adds the body to the create snippet params
func (o *CreateSnippetParams) SetBody(body *models.SnippetEntity) {
	o.Body = body
}

// WriteToRequest writes these params to a swagger request
func (o *CreateSnippetParams) WriteToRequest(r runtime.ClientRequest, reg strfmt.Registry) error {

	if err := r.SetTimeout(o.timeout); err != nil {
		return err
	}
	var res []error
	if o.Body != nil {
		if err := r.SetBodyParam(o.Body); err != nil {
			return err
		}
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}
